...
//To monitor if the user already won
//If one of these is true, the player won
...
function setGame(){
	...
	for (let r = 0; r < rows; r++)
	{
		...
		{
			// This code is to creat a tile through creating div elements
...
window.onload = function(){
	// setGame() is called to be executed upon load
	setGame();
}
...
function handleSlide(e){
	...
	if (["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(e.code))
	{
		...
		//prevents default behavior which is scrolling on key down
		e.preventDefault();
	...
	if (hasLost())
	{
		setTimeout(() =>
		{
			alert("Game Over! The game will now restart.");
			restartGame();
...
function slideLeft(){
	...
	for (let r = 0; r < rows; r++)
	{
		...
		//line for animation
		let originalRow = row.slice(); //initial position

		// Slide function to merge files with the same values
		row = slide(row);
		// Update the row with the merged tiles
		board[r] = row;

		// This loop updates the ids and color of all tiles
		for (let c = 0; c < columns; c++)
		{
			let tile = document.getElementById(r.toString() + "-" + c.toString());
			let num = board[r][c];
			updateTile(tile, num); // update color using fx
...
function slideUp(){
	for (let c = 0; c < columns; c++)
	{
		...
		//Updates the tile's id and color
		for (let r = 0; r < rows; r++)
		{
			board[r][c] = row[r];
			let tile = document.getElementById(r.toString() + "-" + c.toString());
...
function slideDown(){
	for (let c = 0; c < columns; c++)
	{
		...
		//documents original rows
		let originalRow = row.slice();

		row.reverse();
		row = slide(row);
		row.reverse(); //after merging, set back to original sequence by reversing

		//checks which tiles changed position
		let changedIndices = [];

		//records new position of the tiles
		for (let r = 0; r < rows; r++)
		{
